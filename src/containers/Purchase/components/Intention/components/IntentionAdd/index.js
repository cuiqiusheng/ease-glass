...
class IntentionAdd extends Component {
  ...
  constructor(props) {
    ...
    this.columns = [
      { title: '序号', dataIndex: 'a', render: (text, record) => this.renderColumns(text, record, 'a') },
      { title: '产品', dataIndex: 'b', render: (text, record) => this.renderColumns(text, record, 'b') },
      { title: '本地库存数量', dataIndex: 'c', render: (text, record) => this.renderColumns(text, record, 'c') },
      { title: '需求数量', dataIndex: 'd', render: (text, record) => this.renderColumns(text, record, 'd') },
      {
        title: '',
        dataIndex: 'e',
        render: (text, record) => (
    ...
    return (
      <div className={style.wrapper}>
        <div className={classnames('clearfix', 'content-header', 'p20')}>
          <div className={classnames('fl')}>新增采购意向单</div>
        </div>
...
IntentionAdd.propTypes = propTypes
const IntentionAddForm = Form.create()(IntentionAdd)
